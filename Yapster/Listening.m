...
//  Yapster
//
//  Created by Abu-Bakar Bah on 5/24/14.
...
@synthesize json;
@synthesize responseBodyFollowing;
...
- (void)viewDidLoad
{
    ...
    label_following_count.text = [NSString stringWithFormat:@"%.0f", following_count];
    [label_following_count sizeToFit];
    label_following_count.frame = CGRectMake(label_following_count.frame.origin.x, label_following_count.frame.origin.y+9, label_following_count.frame.size.width, label_following_count.frame.size.height);
    
    CGFloat constrainedWidth = 280.0f;
    CGSize sizeOfText;
    
    sizeOfText = [[NSString stringWithFormat:@"%f", following_count] sizeWithFont:[UIFont fontWithName:@"Helvetica Neue" size:16] constrainedToSize:CGSizeMake(constrainedWidth, CGFLOAT_MAX)];
    
    label_following.frame = CGRectMake(label_following_count.frame.size.width+20, label_following.frame.origin.y, label_following.frame.size.width, label_following.frame.size.height);
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (userFollowing.count == 0) {
        ...
        NSURL *the_url = [[NSURL alloc] initWithString:@"http://api.yapster.co/api/0.0.1/users/profile/following/"];
        ...
        responseBodyFollowing = [[NSString alloc] initWithData:returnData encoding:NSUTF8StringEncoding];
    ...
    if (connection == connection1) {
        ...
        if (json_following.count > 0) {
            for (int i = 0; i < json_following.count; i++) {
                ...
                last_user_id = [[[json_following objectAtIndex:i] objectForKey:@"follower_request_id"] doubleValue];
    ...
    if (aScrollView == usersTable) {
        ...
        if(y > h + reload_distance) {
            ...
            if (isLoadingMorePosts == false) {
                ...
                if ([userFollowing count] >= (numLoadedPosts-1)) {
                    DLog(@"loading...");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == usersTable) {
        ...
        cell.label_name.text = [NSString stringWithFormat:@"%@ %@", [[userFollowing objectAtIndex:indexPath.row] objectForKey:@"first_name"], [[userFollowing objectAtIndex:indexPath.row] objectForKey:@"last_name"]];
        [cell.btn_username setTitle:[NSString stringWithFormat:@"@%@", [[userFollowing objectAtIndex:indexPath.row] objectForKey:@"username"]] forState:UIControlStateNormal];
        
        cell.user_photo.tag = [[[userFollowing objectAtIndex:indexPath.row] objectForKey:@"id"] intValue];
        cell.btn_username.tag = [[[userFollowing objectAtIndex:indexPath.row] objectForKey:@"id"] intValue];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == usersTable) {
        ...
        double user_id = [[[userFollowing objectAtIndex:row] objectForKey:@"id"] intValue];
