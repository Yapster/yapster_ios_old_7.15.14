...
-(void)createDomain
{
    SimpleDBCreateDomainRequest *createDomainRequest = [[SimpleDBCreateDomainRequest alloc] initWithDomainName:domainName];
...
-(void)putAttributes
{
    NSMutableArray *attributes = [[NSMutableArray alloc] initWithCapacity:200];
    
    for (int i = 0; i < 200; i++) {
        NSString                     *att                 = [NSString stringWithFormat:@"attribute-%d", i];
        NSString                     *val                 = [NSString stringWithFormat:@"value-%d", i];
        SimpleDBReplaceableAttribute *replacableAttribute = [[SimpleDBReplaceableAttribute alloc] initWithName:att andValue:val andReplace:YES];
