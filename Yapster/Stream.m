...
@synthesize responseBodySearchYaps;
...
- (void)viewDidLoad
{
    ...
    loadingMoreData.frame = CGRectMake(loadingMoreData.frame.origin.x, [[UIScreen mainScreen] bounds].size.height-50, loadingMoreData.frame.size.width, loadingMoreData.frame.size.height);
    ...
    if ([searchBox respondsToSelector:@selector(setAttributedPlaceholder:)]) {
        UIColor *color = [UIColor whiteColor];
        searchBox.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"Search Stream" attributes:@{NSForegroundColorAttributeName: color}];
    }
    ...
    theTable.frame = CGRectMake(self.view.bounds.origin.x, self.view.bounds.origin.y+90, self.view.bounds.size.width, self.view.bounds.size.height-70);
    ...
    loadingMoreData.frame = CGRectMake(loadingMoreData.frame.origin.x, theTable.frame.size.height+20, loadingMoreData.frame.size.width, loadingMoreData.frame.size.height);
    
    loadingMoreData.hidden = YES;
    
    message.frame = CGRectMake(20, 275, message.frame.size.width, message.frame.size.height);
    
    //refreshControl[theTable addSubview:refreshFeed];
...
-(void)connectionDidFinishLoading:(NSURLConnection *) connection {
    if (connection == connection1) {
        ...
        //dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, (unsigned long)NULL), ^(void) {
            ...
            if (json.count > 0) {
                if (true) {
                    ...
                    if ((isLoadingMorePosts && !isRefresh && !isSearch2) || isLoadingMoreSearchPosts) {
                ...
                if (numLoadedPosts < postAmount) {
                    message.text = @"Your stream is currently empty.";
                    message.hidden = NO;
        ...
        NSData *data = [responseBodySearchYaps dataUsingEncoding:NSUTF8StringEncoding];
...
-(IBAction)cancelMenuSearch:(id)sender {
    if ([menuSearchBox.text isEqualToString:@""]) {
        ...
        menuSearchBox.text = @"";
        [menuSearchBox becomeFirstResponder];
    }
}
...
-(IBAction)cancelSearch:(id)sender {
    if ([searchBox.text isEqualToString:@""]) {
        ...
        playAllBtn.frame = CGRectMake(206, playAllBtn.frame.origin.y, playAllBtn.frame.size.width, playAllBtn.frame.size.height);
...
-(IBAction)doSearch:(id)sender {
    userFeed = [NSMutableArray array];
    [userFeed removeAllObjects];
    
    BOOL hashtagsSearch = false;
    BOOL usersSearch = false;
    ...
    if (![search_text isEqualToString:@""]) {
        ...
        if (usersSearch) {
            if (!hashtagsSearch) {
                the_url = [[NSURL alloc] initWithString:@"http://www.api.yapster.co/api/0.0.1/search/stream/user_handles/search/"];
                ...
                the_url = [[NSURL alloc] initWithString:@"http://www.api.yapster.co/api/0.0.1/search/stream/hashtags_and_user_handles/search/"];
        ...
        else if (hashtagsSearch) {
            the_url = [[NSURL alloc] initWithString:@"http://www.api.yapster.co/api/0.0.1/search/stream/hashtags/search/"];
            ...
            the_url = [[NSURL alloc] initWithString:@"http://www.api.yapster.co/api/0.0.1/search/stream/text/search/"];
        ...
        responseBodySearchYaps = [[NSString alloc] initWithData:returnData encoding:NSUTF8StringEncoding];
        ...
        if (connection7) {
            if (!cameFromPlayerScreen && !isRefresh) {
                // terminate all pending download connections
                NSArray *allDownloads = [self.imageDownloadsInProgress allValues];
                [allDownloads makeObjectsPerformSelector:@selector(cancelDownload)];
...
-(IBAction)playAll:(id)sender {
    ...
    if (userFeed.count > 0) {
        for (int i = 0; i < userFeed.count; i++) {
            ...
            [playlistDic setObject:@"home_stream" forKey:@"came_from"];
...
- (void)startTheBackgroundJob {
    ...
    [self performSelectorOnMainThread:@selector(loadMorePosts:) withObject:nil waitUntilDone:NO];
    ...
    if (tableView == theTable) {
        ...
        if (nodeCount > 0 && user_photo_entries.count == nodeCount)
        {
            ...
            if (userFeedData.web_link_flag) {
                ...
                [cell.web_link_btn setLineBreakMode:NSLineBreakByTruncatingTail];
            ...
            case 0:
                switch (indexPath.row) {
                    case 0:
                        ...
                        break;
                    case 1:
                        theIconImage = [UIImage imageNamed:@"bttn_stream grn.png"];
                        break;
                    case 2:
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == menuTable) {
                    ...
                    switch (indexPath.row) {
                        ...
                        case 2: {
                            if (playerActive) {
                                ...
                                NSString *yap_picture_path = [[sharedManager.sessionCurrentPlaylist objectAtIndex:sharedManager.currentYapPlaying]  valueForKey:@"picture_path"];
                                NSString *yap_picture_cropped_path = [[sharedManager.sessionCurrentPlaylist objectAtIndex:sharedManager.currentYapPlaying] valueForKey:@"picture_cropped_path"];
                                ...
                                if (yap_picture_flag) {
                                    ...
                                    [[sharedManager.sessionCurrentPlaylist objectAtIndex:sharedManager.currentYapPlaying] setValue:big_photo forKey:@"big_photo"];
                                }
                                */
        ...
        playerScreenVC.cameFrom = @"home_stream";
...
- (void)scrollViewDidScroll:(UIScrollView *)aScrollView {
    if (aScrollView == theTable) {
        ...
        if (y_offset >= 41) {
            ...
            //theTable.frame = CGRectMake(self.view.bounds.origin.x, self.view.bounds.origin.y+88, self.view.bounds.size.width, self.view.bounds.size.height-70);
        }
        else {
            menu_btn.hidden = NO;
        ...
        if (bottomEdge >= aScrollView.contentSize.height-5) {
            ...
            if (isLoadingMorePosts == false) {
                ...
                DLog(@"userFeed.count %lu, numLoadedPosts %d", (unsigned long)userFeed.count, numLoadedPosts);
...
-(IBAction)toggleSearchBox:(id)sender {
    if (!searchBoxVisible) {
        playAllBtn.frame = CGRectMake(245, playAllBtn.frame.origin.y, playAllBtn.frame.size.width, playAllBtn.frame.size.height);
